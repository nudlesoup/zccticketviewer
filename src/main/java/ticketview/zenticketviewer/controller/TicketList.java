...
public class TicketList {
	...
	*/
	private void displayTicketList(int page)
	{	
		//Display title
		...
		for(int i=0; i<ticketArray.length; i++)
			displayTicketInformation(ticketArray[i]);
		displayFooter();
	}
	...
	*/
    ...
    {
    	String str = ticket.getSubject();
    	//Limit subject length to 50 chars
    	if (str.length() >50)
    		str = str.substring(0,46) + "...";
    	//Align left
    	System.out.format("| %-10s | %-10s | %-50s | %-10s | %-10s | %-25s |\n", ticket.getId()+"", ticket.getType(), str, ticket.getPriority(), ticket.getStatus(), ticket.getCreatedAt());
    }
